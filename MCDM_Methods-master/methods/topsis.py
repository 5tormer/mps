...
def prepare_info(res_dataset):
    ...
    max_info = f'<{browsers[max_ind]}> is the best. It has score = {res_dataset[max_ind]:.4f}'
    min_info = f'<{browsers[min_ind]}> is the worst. It has score = {res_dataset[min_ind]:.4f}'
    res = [f'<{browsers[i]}> has score: {x:.4f}' for i, x in enumerate(res_dataset)]
    res.append(max_info)
    res.append(min_info)
    res = '\n'.join(res)
    print('\n---------TOPSIS METHOD---------\n')
